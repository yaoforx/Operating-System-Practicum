...
void check_priority(struct process * current){
    ...
    if(now - current->last_run > current->remain_quantum) {
        //means that this quantum has used up
        current->priority += (current->priority == MAX_PRIORITY - 1) ? 0 : 1;
        current->remain_quantum = quantum[current->priority ];
    } else {
        // not used up
        current->remain_quantum -= now - current->last_run;
    }
...
static void proc_wakeup(struct process *p){
    ...
    if (p != proc_current) {
#ifdef HW_MLFQ
        //Improve its priority if possible.
        p->priority -= (p->priority == MAX_PRIORITY - 1) ? 1 : 0;
 ...
 */
    ...
    mtlQueue->q = malloc(MAX_PRIORITY * sizeof(struct queue *));
    if(!mtlQueue || mtlQueue->q == NULL) {
        perror("Multi-Level Queue Failed!\n");
        sys_exit(1);
    }

     for(int i = 0; i < mtlQueue->level; i++) {
         mtlQueue->q[i] = malloc(sizeof(struct queue));
